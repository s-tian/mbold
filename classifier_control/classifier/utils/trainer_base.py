...
class BaseTrainer():
    ...
    def check_nan_grads(self, grads=None):
    ...
    def nan_grads_hook(self, inputs, output, losses, epoch):
        non_none = filter(lambda x: x[1].grad is not None, self.model.named_parameters())
        if self.check_nan_grads(non_none):
            ...
            non_nan_parameters = [n for n, v in
                              filter(lambda x: not torch.isnan(x[-1].grad).any(), self.model.named_parameters())]
            grad_dict = self.print_nan_grads()
            print('nan_parameters', nan_parameters)
            print('non_nan_parameters', non_nan_parameters)

...
class TabletopReacher(BaseMujocoEnv, SawyerXYZEnv):
    ...
    def set_goal(self, goal_obj_pose, goal_arm_pose):
        ...
        super(TabletopReacher, self).set_goal(goal_obj_pose, goal_arm_pose)

    def get_distance_score(self):
        """
        :return:  mean of the distances between all objects and goals
        """
        # self._goal_obj_pose contains gripper xyz!
        return np.linalg.norm(self._goal_obj_pose - self.get_endeff_pos())

    def has_goal(self):
        return True

    def goal_reached(self):
        og_pos = self._obs_history[0]['gripper'][:2]
        distance = np.linalg.norm(og_pos - self.get_endeff_pos()[:2])
